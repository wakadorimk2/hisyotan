...
class ZombieOverlayManager {
    ...
    if (this.warningOverlay) {
      ...
      if (this.useCssForWarningOverlay) {
        this.warningOverlay.style.cssText = `
          ...
          width: 100%;
          height: 100%;
          background-color: rgba(255, 0, 0, ${opacity * 0.2});
          box-shadow: inset 0 0 50px rgba(255, 0, 0, ${opacity * 0.5});
          opacity: ${opacity};
          transition: opacity 0.5s ease-in-out;
          pointer-events: none;
          z-index: 9991;
          animation: zombie-warning-pulse 2s infinite ease-in-out;
        `;
      } else {
        // 通常の画像ベースの表示
  ...
  testOverlay() {
    ...
    this.showDetection(testData.yolo, testData.resnet_alive);
  }
}

// シングルトンインスタンス
const zombieOverlayManager = new ZombieOverlayManager();

export default zombieOverlayManager; 
...
class ZombieClassifier:
    ...
    def prepare_data(self, batch_size=8):
        ...
        if not subdirs:
            raise FileNotFoundError(
                f"データパス {self.data_path} 内にクラスディレクトリが見つかりません。"
            )

        print(f"データディレクトリ: {[d.name for d in subdirs]}")

        # データセットの作成
        train_dataset = ZombieDataset(
            self.data_path, transform=self.train_transform, train=True
        )
        valid_dataset = ZombieDataset(
            self.data_path, transform=self.valid_transform, train=False
        )
...
def main():
    ...
    try:
        test_img = list(
            Path("../data/datasets/zombie_classifier/zombie").glob("*.png")
        )[0]
        print(f"🧪 テスト画像での推論: {test_img}")
        pred_class, prob = classifier.predict_image(test_img)
    except IndexError:
        print(
            "警告: テスト画像が見つかりませんでした。モデルは正常に学習されています。"
        )

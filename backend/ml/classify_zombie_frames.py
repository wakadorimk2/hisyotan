...
def classify_images(
    input_dir, output_base_dir, model_path, confidence_threshold=CONFIDENCE_THRESHOLD
):
    """
    ディレクトリ内の画像を分類する関数
    ...
    # 入力ディレクトリ内の全画像を列挙
    image_paths = list(Path(input_dir).glob("*.jpg")) + list(
        Path(input_dir).glob("*.png")
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="7 Days to Dieのゲームフレームをゾンビ/非ゾンビに分類します"
    )
    parser.add_argument(
        "--input_dir",
        "-i",
    ...
    # 分類処理を実行
    classify_images(args.input_dir, args.output_dir, args.model_path, args.threshold)

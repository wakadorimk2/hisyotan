...
class ZombieService:
        ...
        try:
            ...
            try:
                ...
                if target_classes_str:
                    ...
                    target_classes = [
                        int(cls_id.strip())
                        for cls_id in target_classes_str.split(",")
                        if cls_id.strip()
                    ]
                    logger.info(f"環境変数から対象クラスを設定: {target_classes}")
            except Exception as e:
                logger.warning(
                    f"対象クラスの設定中にエラー: {e}、デフォルト値を使用します"
                )
            ...
            logger.info(
                f"ゾンビ検出サービスを初期化しました（対象クラス: {target_classes}）"
            )
            return True
    ...
    async def start_monitoring(self) -> Optional[asyncio.Task[Any]]:
        ...
        if not self.is_initialized:
            ...
            if not success:
                logger.error(
                    "ゾンビ検出サービスの初期化に失敗したため、監視を開始できません"
                )
                return None
    ...
    async def stop_monitoring(self) -> bool:
        ...
        if self.detector is None:
            logger.warning(
                "ゾンビ検出器が初期化されていないため、停止する必要はありません"
            )
            return True

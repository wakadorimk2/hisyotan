...
class FrameExtractor:
    ...
    def extract_frames(self) -> bool:
        ...
        logger.info(f"抽出間隔: {self.frame_interval}フレームごと（約 {self.frame_interval/fps:.2f}秒に1枚）")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", "-o", help="出力ディレクトリのパス（指定しない場合は自動生成）")
    parser.add_argument("--interval", "-n", type=int, default=30, 
                        help="フレーム抽出間隔（デフォルト: 30、つまり30FPSの動画で1秒に1枚）")
    parser.add_argument("--prefix", "-p", default="frame_", 

...
class VoicePlaybackState:
    ...
    def is_message_duplicate(self, message_type: str, message: str, cooldown: float = 3.0) -> bool:
        """
...
class VoiceService:
    ...
    def get_random_dialogue(self, dialogue_type: str, subtype: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        # サブタイプがなく、直接リストの場合
            ...
            logger.warning(f"指定された対話データが見つかりません: {dialogue_type}/{subtype}")
            return None
    ...
    def safe_play_voice(self, text: str, speaker_id: Optional[int] = None, 
                       speed: float = 1.0, pitch: float = 0.0, 
                       intonation: float = 1.0, volume: float = 1.0, 
                       force: bool = False, message_type: str = "default") -> Optional[str]:
        """
        ...
        with self.voice_lock:
            ...
            if not force and current_time - _playback_state.last_voice_time < self.settings.VOICE_COOLDOWN:
    ...
    def play_dialogue(self, dialogue_type: str, subtype: Optional[str] = None, 
                     force: bool = False, message_type: Optional[str] = None) -> Optional[str]:
        """
        対話データから選択したセリフを再生
        ...
        voice_preset = self.settings.VOICE_PRESETS.get(emotion, {"pitch": 0.0, "intonation": 1.0, "speed": 1.0})
    ...
    def _speak(self, text: str, speaker_id: int = 0, speed: float = 1.0, 
              pitch: float = 0.0, intonation: float = 1.0, volume: float = 1.0) -> Optional[str]:
        """
        ...
        try:
            ...
            query_response = requests.post(
                f"{self.settings.VOICEVOX_HOST}/audio_query",
                params=params
            )
